...
export class MultiSelectLookupControl implements ComponentFramework.StandardControl<IInputs, IOutputs> {
    ...
    OpenLookups = () => {
        ...
        let filterXml = `<filter type="and"></filter>`;
        let attribute = this.context.parameters.conditionAttribute.raw;
        let fieldLogicalName = this.context.parameters.filterByField.raw;
        let allowEmptyFilter = this.context.parameters.allowEmptyFilter.raw === "True";
        let value = null;
        if (fieldLogicalName) {
            switch (Xrm.Page.getAttribute(fieldLogicalName)?.getAttributeType()) {
                case "lookup":
                    value = Xrm.Page.getAttribute(fieldLogicalName)?.getValue()?.[0].id;
                    break;
                default:
                    value = Xrm.Page.getAttribute(fieldLogicalName)?.getValue();
                    break;
            }
        }
        if (attribute) {
            if (value) {
                filterXml = `<filter type='and'><condition attribute='${attribute}' operator='eq' value='${value}'/></filter>`;
            } else if (!allowEmptyFilter) {
                filterXml = `<filter type='and'><condition attribute='${attribute}' operator='eq' value='00000000-0000-0000-0000-000000000000'/></filter>`;
            }
        }
        let primaryEntity = ((this.context) as any)?.page?.entityTypeName;
        let primaryEntityId = ((this.context) as any)?.page?.entityId;
    ...
    public updateView(context: ComponentFramework.Context<IInputs>): void {
        ...
        for (let key in records) {
            ...
            recordOfArray.push({
                ...
                entityType: type,
                name: (record._record.fields[record._primaryFieldName]?.value) || "(Nenhum Nome)"
            })
        }
